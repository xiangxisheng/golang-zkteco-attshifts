...
## 4. 已修复/改进逻辑
- **日期截断问题**: 已修复月份最后一天的查询问题。
- **旷工与请假自动抵扣**: 
    - 逻辑调整：现在系统会先计算出勤与应出的差值作为“旷工”。如果当天存在请假记录，系统会自动用请假天数抵扣旷工天数。
    - 单元格显示：
        - 正常出勤显示出勤天数（如 `1`）。
        - 旷工显示 `旷`。
        - 请假根据类型显示简写：`检` (产检假)、`病` (病假)、`事` (事假)、`产` (产假)、`年` (年假)。
        - 部分出勤+请假会组合显示（如 `0.5病`）。
## 5. 配置项说明
- **`weekend`**: 定义周末。
    - 类型：整数数组（0 代表周日，1-6 代表周一至周六）。
    - 默认值：`[0, 6]` (周六和周日)。
    - 影响：定义为周末的日期不计入“应出勤”范围，不会产生旷工。若改为 `[0]`，则周六将被视为正常工作日，未出勤且未请假将计为旷工。
